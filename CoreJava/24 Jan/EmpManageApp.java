...
class Clerk extends Emp implements Serializable{
...
class CSVReader {
    ...
    public static void fillHashMapFromCSV() throws Exception {
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream("employees.ser"))) {
            EmpManageApp.emp = (HashMap<Integer, Emp>) ois.readObject();
            for(Emp e : EmpManageApp.emp.values()) {
                if(e.designation.equals("CEO")) {
                    CEO.setCeoCreated(true);
                    break;
                }
            }
            Emp.countEmp = EmpManageApp.emp.size();
        } catch(Exception e) {
            e.printStackTrace();
        }   
    }
...
class WriteToCSV
{
    public static void serializeAndWrite(HashMap<Integer, Emp> emp) throws Exception {
    ...
    public static void deleteFromCSV(HashMap<Integer, Emp> emp) throws Exception {    
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("employees.ser"))) {
            ...
            switch (ch1) {
                case 1:
                    do {
                        ...
                        WriteToCSV.serializeAndWrite(EmpManageApp.emp);
                    } while (ch2 != 5);
                    break;
